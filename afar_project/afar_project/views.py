...
def frc_system(request):
    if request.method == 'POST':
        # Assuming your form field name is 'excel_file'
        uploaded_file = request.FILES['file']
...
<<<<<<< HEAD
        if len(existing_df['Financial Year'])>len(uploaded_df['Financial Year']):
            ...
            file=uploaded_df# Concatenate DataFrames
...
def frc_asset_schedule(request):
    ...
    # Read the CSV files into dataframes, skipping the header row
        ...
        # Write the DataFrame to a new CSV file
        result_df.to_excel('csv_path/frc_asset_schedule.xlsx', index=False, header=False)

        df_asset_schedule = pd.read_excel('csv_path/frc_asset_schedule.xlsx')
